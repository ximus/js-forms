...
function checkboxGetSiblings(checkbox: HTMLInputElement) {
  ...
  return [
    ...scope.querySelectorAll(`input[type=checkbox][name="${checkbox.name}"]`),
  ];
}

function hasMultipleValues(target: Target) {
...
function getMultiple(target: Target) {
  const { type } = target;
...
export function set(target: HTMLElement, value: any) {
